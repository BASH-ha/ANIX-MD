...
function patchSocket() {
    if (!fs.existsSync(SOCKET_FILE)) { console.log('[ANIX-MD] socket.js not found, skipping'); return; }
    let code = fs.readFileSync(SOCKET_FILE, 'utf-8');

    if (code.includes('// [PATCHED] event buffer disabled')) {
        console.log('[ANIX-MD] socket.js already patched');
        return;
    }
    ...
    if (patched) {
        ...
        console.log('[ANIX-MD] ✅ socket.js patched - event buffering disabled');
    } else {
        console.log('[ANIX-MD] socket.js - no matching patterns found (may not need patching in this version)');
    }
}

function patchChats() {
    if (!fs.existsSync(CHATS_FILE)) { console.log('[ANIX-MD] chats.js not found, skipping'); return; }
    let code = fs.readFileSync(CHATS_FILE, 'utf-8');

    if (code.includes('Skipping AwaitingInitialSync') || code.includes('// [PATCHED] skip sync')) {
        console.log('[ANIX-MD] chats.js already patched');
        return;
    }
    ...
    if (code.includes(target7)) {
        ...
        console.log('[ANIX-MD] ✅ chats.js patched (7.x) - AwaitingInitialSync bypassed');
        return;
    }
    ...
    if (syncBlock6.test(code)) {
        ...
        fs.writeFileSync(CHATS_FILE, code, 'utf-8');
        console.log('[ANIX-MD] ✅ chats.js patched (6.x fallback) - AwaitingInitialSync bypassed');
        return;
    }

    console.log('[ANIX-MD] chats.js - no matching patterns found');
}

function patchMessagesRecv() {
    if (!fs.existsSync(MESSAGES_RECV_FILE)) { console.log('[ANIX-MD] messages-recv.js not found, skipping'); return; }
    let code = fs.readFileSync(MESSAGES_RECV_FILE, 'utf-8');

    if (code.includes('// silenced mex newsletter')) {
        console.log('[ANIX-MD] messages-recv.js already patched');
        return;
    }
    ...
    if (patched) {
        ...
        console.log('[ANIX-MD] 🔇 Silenced newsletter notification warnings');
    } else {
        console.log('[ANIX-MD] messages-recv.js - newsletter patterns not found (may not exist in this version)');
    }
}
...
function patchSessionCipher() {
    ...
    if (!SESSION_CIPHER_FILE) { console.log('[ANIX-MD] session_cipher.js not found, skipping'); return; }

    let code = fs.readFileSync(SESSION_CIPHER_FILE, 'utf-8');
    if (code.includes('// silenced decrypt errors')) {
        console.log('[ANIX-MD] session_cipher.js already patched');
        return;
    }
    ...
    console.log('[ANIX-MD] 🔇 Silenced libsignal decrypt error logs');
}

console.log('[ANIX-MD] 🚀 Applying patches...');
patchSocket();
patchChats();
patchMessagesRecv();
patchSessionCipher();
console.log('[ANIX-MD] ✅ All patches applied successfully!');
console.log('');
console.log('═══════════════════════════════════════');
console.log('     ANIX-MD is ready - Bashiri');
console.log('     📢 Join BOT HUB on WhatsApp');
console.log('═══════════════════════════════════════');
console.log('');
