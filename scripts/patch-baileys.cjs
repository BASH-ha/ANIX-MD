...
console.log('');
console.log('═══════════════════════════════════════');
console.log('     🤖 BOT HUB by Bashiri 🤖');
console.log('═══════════════════════════════════════');
console.log('');

function patchSocket() {
    if (!fs.existsSync(SOCKET_FILE)) { console.log('[BOT HUB] socket.js not found, skipping'); return; }
    let code = fs.readFileSync(SOCKET_FILE, 'utf-8');

    if (code.includes('// [PATCHED] event buffer disabled')) {
        console.log('[BOT HUB] socket.js already patched');
        return;
    }
    ...
    if (patched) {
        ...
        console.log('[BOT HUB] ✅ socket.js patched - event buffering disabled');
    } else {
        console.log('[BOT HUB] socket.js - no matching patterns found (may not need patching in this version)');
    }
}

function patchChats() {
    if (!fs.existsSync(CHATS_FILE)) { console.log('[BOT HUB] chats.js not found, skipping'); return; }
    let code = fs.readFileSync(CHATS_FILE, 'utf-8');

    if (code.includes('Skipping AwaitingInitialSync') || code.includes('// [PATCHED] skip sync')) {
        console.log('[BOT HUB] chats.js already patched');
        return;
    }
    ...
    if (code.includes(target7)) {
        ...
        console.log('[BOT HUB] ✅ chats.js patched (7.x) - AwaitingInitialSync bypassed');
        return;
    }
    ...
    if (syncBlock6.test(code)) {
        ...
        fs.writeFileSync(CHATS_FILE, code, 'utf-8');
        console.log('[BOT HUB] ✅ chats.js patched (6.x fallback) - AwaitingInitialSync bypassed');
        return;
    }

    console.log('[BOT HUB] chats.js - no matching patterns found');
}

function patchMessagesRecv() {
    if (!fs.existsSync(MESSAGES_RECV_FILE)) { console.log('[BOT HUB] messages-recv.js not found, skipping'); return; }
    let code = fs.readFileSync(MESSAGES_RECV_FILE, 'utf-8');

    if (code.includes('// silenced mex newsletter')) {
        console.log('[BOT HUB] messages-recv.js already patched');
        return;
    }
    ...
    if (patched) {
        ...
        console.log('[BOT HUB] 🔇 Silenced newsletter notification warnings');
    } else {
        console.log('[BOT HUB] messages-recv.js - newsletter patterns not found (may not exist in this version)');
    }
}
...
function patchSessionCipher() {
    ...
    if (!SESSION_CIPHER_FILE) { console.log('[BOT HUB] session_cipher.js not found, skipping'); return; }

    let code = fs.readFileSync(SESSION_CIPHER_FILE, 'utf-8');
    if (code.includes('// silenced decrypt errors')) {
        console.log('[BOT HUB] session_cipher.js already patched');
        return;
    }
    ...
    console.log('[BOT HUB] 🔇 Silenced libsignal decrypt error logs');
}

console.log('[BOT HUB] 🚀 Applying patches...');
patchSocket();
patchChats();
patchMessagesRecv();
patchSessionCipher();
console.log('[BOT HUB] ✅ All patches applied successfully!');
console.log('');
console.log('═══════════════════════════════════════');
console.log('     BOT HUB is ready - Bashiri');
console.log('═══════════════════════════════════════');
console.log('');
